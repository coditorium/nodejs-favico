...
const checkIfMasterPictureHasNotChanged = (dirPath, config) =>
  Promise.all([
    ...
    .then(logIfNot(config.log, `Cache directory ${dirPath} invalidated. Master picture was modified.`));
...
const checkIfConfigIsNotChanged = (dirPath, config) =>
  util.readJson(`${dirPath}/meta.json`)
    ...
    .then(logIfNot(config.log, `Configuration changed. Cache dir ${dirPath} invalidated.`));

const checkIfDirExists = (dirPath, config) =>
  util.dirExists(dirPath)
    .then(logIfNot(config.log, `Cache directory does not exist '${dirPath}'`));
...
const isChanged = (dirPath, config) =>
  checkIfDirExists(dirPath, config)
    ...
    .catch((err) => {
      config.log(`Cache directory ${dirPath} invalidated. Cause:`, err);
      return false;
    });
